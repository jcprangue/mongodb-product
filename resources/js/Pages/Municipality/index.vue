<template>
  <div>
    <div class="flex justify-between">
      ...
      <inertia-link class="btn-indigo" :href="route('LGUs.create')">
        <span>Create</span>
        <span class="hidden md:inline">LGU</span>
      </inertia-link>
    </div>
    ...
    <table class="w-full my-4 whitespace-no-wrap bg-white">
      <tr class="font-bold text-left">
        <th class="p-4">#</th>
        <th class="p-4">LGUs</th>
        <th class="p-4 text-center">Action</th>
      </tr>

      <tr
        v-for="(row, i) of municipalities"
        :key="row.id"
        ...
        <td class="p-3 w-64">
          <div class="flex place-content-center">
            ...
            <div class="grid">
              <inertia-link
                class="focus:text-indigo-500"
                :href="route('brgy.index', { id: row.id })"
                tabindex="-1"
              >
...
export default {
  ...
  watch: {
    filterForm: {
      handler: throttle(function () {
        this.$inertia.replace(this.route("LGUs.index", this.filterForm));
      }, 150),
      deep: true,
    },
  },
  methods: {
    submit() {
      this.$inertia.get(this.route("LGUs.index"), {
