  ...
  <app-layout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        ...
        <inertia-link
          class="text-indigo-400 hover:text-indigo-600"
          :href="route('records-details.index', { id: procurement.id })"
        >
...
export default {
  ...
  props: {
    ...
    procurement: { type: [Object, Array], default: () => {} },
  },
  data() {
    return {
      sending: false,
      form: {
        procurement_record_id: this.procurement.id,
  ...
  methods: {
    submit() {
      this.sending = true;
      this.$inertia
        .post(this.route("records-details.store"), this.form)
        .then((response) => {
          this.sending = false;
        });
    },
